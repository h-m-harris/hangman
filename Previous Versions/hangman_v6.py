...
def game_options():
    ...
    # Create text and rectangles with border for each difficulty using "desplay_text" method
...
def game_endings(game_ending):
    while True:
        ...
        if game_ending == 1:
            # Create text Lost
            display_text(" You Lost! ", x=250, y=350, font=title_font)
        ...
        if game_ending == 2:
            # Create text Won!
            display_text(" You Won!  ", x=250, y=350, font=title_font)
            # Display play/quit buttons
            play_again()
            pygame.display.update()
